...
import static org.util.DirectionUtil.getDirectionToTarget;
import static org.util.DirectionUtil.getOppositeDirection;
...
public class MoveGen {
  ...
  private void generateTaboo() {
    for (int startSquare = 0; startSquare < 64; startSquare++) {
      ...
      switch (getType(piece)) {
        case PAWN -> PawnGen.generateOpponentPawnMoves(startSquare, boardState, moveGeneratorState);
        case BISHOP, ROOK, QUEEN -> SlidingGen.generateOpponentSlidingMoves(startSquare, piece, boardState, moveGeneratorState);
        case KNIGHT -> KnightGen.generateOpponentKnightMoves(startSquare, moveGeneratorState, boardState);
      }
    }
    KingGen.generateOpponentKingMoves(boardState, moveGeneratorState);
  }

  private void generateFriendlyMoves() {
    for (int startSquare = 0; startSquare < 64; startSquare++) {
      int piece = boardState.getSquare(startSquare);
      ...
      switch (getType(piece)) {
        case PAWN -> PawnGen.generateFriendlyPawnMoves(startSquare, boardState, moveGeneratorState);
        case BISHOP, ROOK, QUEEN -> SlidingGen.generateFriendlySlidingMoves(startSquare, piece, boardState, moveGeneratorState);
        case KNIGHT -> KnightGen.generateFriendlyKnightMoves(startSquare, moveGeneratorState);
      }
    }
      ...
      if (isType(pinnedPiece, PAWN)) {
        ...
        for (String move : moveGeneratorState.getMoves()) {
          ...
          if (directionToPinningPiece == NORTH || directionToPinningPiece == SOUTH) {
            if (getTargetSquare(move) + 8 != getStartSquare(move) && getTargetSquare(move) + 16 != getStartSquare(move) && getTargetSquare(move) - 8 != getStartSquare(move)
                && getTargetSquare(move) - 16 != getStartSquare(move)) {
              movesToRemove.add(move);
            }
            continue;
          }
  ...
  private static List<String> getRemainingLegalMoves(BoardState boardState, MoveGeneratorState moveGeneratorState) {
    ...
    int xDist = X_DIST[friendlyKingPosition][getStartSquare(checkingMove)];
    int yDist = Y_DIST[friendlyKingPosition][getStartSquare(checkingMove)];
    ...
    return remainingLegalMoves.stream().distinct().collect(Collectors.toList());
  }
